...
test('/api/watchers serves stored watcher state', async () => {
  ...
  // "ci" watcher against the upstream repo, and buildChecks() reads it from the repo's
  ...
  try {
    // Readiness-poll instead of a fixed sleep: under full-suite load the server can take
    // longer than 700ms to bind, which made this test flake with ECONNREFUSED.
    let w = null;
    for (let i = 0; i < 40 && w === null; i++) {
      await new Promise(r => setTimeout(r, 250));
      try { w = await (await fetch(`${BASE}/api/watchers`)).json(); } catch (e) {}
    }
    assert.ok(w, 'server did not become ready');
